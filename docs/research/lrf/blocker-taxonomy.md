...
- owner 指向 UI 的 rectangle，CLI 已能读 drawing state，但 R 没跑 Client Mirror First；
...
- hypothesis writer 看到了 reveal / outcome；
- 让写 hypothesis 的 Agent 自己当裁判；
- Reviewer Agent 代替 deterministic judge 判市场结果；
...
- 缺失字段或能力；
- 当前可以继续做什么；
- 下一步需要 owner、C、app-side、data-side 或 R 哪一方处理。

不能用 blocker 掩盖 R 自己的研究偷懒。如果 app 已暴露能力但 R 没用，应标 `R_APP_USAGE_GAP`。如果 R 跳过 entry / exit / stop / failure / no-entry / blind discipline，应标 `R_METHOD_GAP`。
